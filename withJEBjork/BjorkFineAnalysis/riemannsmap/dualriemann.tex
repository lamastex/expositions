...
\begin{document}
\centerline{\bf\large{Riemann surfaces}}
...
\noindent
A complex analytic manifold $X$ of  dimension one is called a Riemann surface.
If $X$ is non-compact one refers to an open Riemann surface, while
$X$ is a closed Riemann surface if it is compact.
The study of open, respectively closed Riemann surface
leads to  different results. When $X$ is closed the maximum principle for analytic functions entails that
the space
$\mathcal O(X)$ of globally defined holomorphic functions
is reduced to constant  function. For an open Riemann surface  $X$ the situation is quite different.
For example, let $\{p_\nu\}$ be a discrete sequence of points on
$X$ snd $\{k_\nu\}$ a sequence of positive  integers then there exists 
$f\in\mathcal O(X)$ which is zero-free outside
the $p$-points, and  for every $\nu$ it has  a zero of order $k_\nu$ at $p_\nu$
Moreover the Cousin problem has a solution for
every pair of open sets $U,V$ whose union is equal to $X$.  It means that for every 
$\phi\in\mathcal O(U\cap V)$
there exist 
$a\in\mathcal O(U)$ and $b\in \mathcal O(V)$
such that
the restriction of $a-b$ to $U\cap V$ is equal to $\phi$.
When $X$ is a  compact Riemann surface
the obstruction to solutions  of the Cousin problem
for a pair of open sets $U,V$ whose union is $X$ turns put to
be described by a $g$-dimensional complex vector space
where $g$ is the genus of $X$.
We treat the compact case in § x while open Riemann surfaces
are studied in § xx.
Closed Riemann surfaces arise via
algebraic equations.
Namely, let ${\bf{C}}[x,y]$ be
the polynomial ring of two variables with complex coefficients.
To each irreducible polynomial $p(x,y)$ we get the algebraic curve
in ${\bf{C}}^2$ defined by $\{p=0\}$ and let us denote it by $S$.
Asume that $p$ is moniic with respect to $y$, i.e. 
\[ 
p(x,y)= y^m+q_{m-1}(x)y^{m-1}+\ldots+q_0(x)
\]
where no special assumption is imposed on the $q$-polynomials except fopr the condition that
$p(x,y)$ is an irredicuble polynomial.
Geberal residue calculus - which actually is valid for
algebraic hypersurfaces in ${\bf{C}}^n$ for every $n\geq 2$
asserts that there exists a $\bar\partial$-closed current of bi-degree $(1,0)$
define dfor test-forms
$\psi^{0,1}$ in the 4-dimensional space attached to
${\bf{C}}^2$ and defined by
\[
 \psi^{0,1}\mapsto \int_S\, \frac{dx}{P'_y}\wedge \psi^{0,1}
\]
Above $S$ is an algebraic curve in
${\bf{C}}^2$
and we remark that the $\bar\partial$-closed current excises in general, i.e. even
when
$S$ contains singular points.
However, via a clöassic local contraction due to Puiseue
there exists  normalization of $S$ which yields a non-singular
curve $\widehat{S}$ and a map $\rho\colon \widehat S\to S$.
Passing to the proactive closure one can also
construct local charts in the sense of Puiseux and in this
way
arrive at a closed Riemann surface $X$
for this one has a map $\rho\colon X\to \bar S$
which in many cases isda homoeomorphiusm but in certain examples it
appears that
for a  finite set of points in $S$ the inverse finger
$\rho^{-1}(s)$ is a finite set in $X$.
However, in all cases the
current (*) yields  via  a pull-back to $X$
s $\bar\partial$-closed differential form
with therefore is called a holomorphic form.
The whole space of holographic 1-forms on $X$ is denoted by
$\Omega(X)$. We shall learn that it is a finite dimensional complex vector spec
whose dimension is equalmto the genus of $X$.
To find a basis for $\Omega(X)$
one must study
under what conditions the special holomorophic 1-form
extends to be holomorphic 
at points in $X$ which belong to $\rho^{-1}(\partial S)$.,
Here one encounters typical calculations during the study of closed Riemann surfaces.
\medskip
...
p(x,y)= y^2-q(x)
\] 
where $q(x)$ is a polynomial on $x$ only and of some odd degree $2m+1$
with $m\geq 1$.
Here we shall learn that
$y$ extends to a meromorphic function oin $X$ with a
pole of
order $2m+1$ at the point at infinity, while $x$ has a pole of order 2.
Dfrom this it follows that the meromorphic differential $dx$ has a pole of order 3
at infinity and
at the same time
$y^{-1}$ has a zero of order $2m+1$. It follows that
$\frac{dx}{y}$ remains holographic at the point at infinity.
If $m\geq 2$ one gets more holomorphic  1-forms on $X$ using
\[
 \omega_\nu= x^\nu\cdot \frac{dx}{y}
\]
where the condition is that
\[
2\nu\leq 2m+1-3\implies \nu\leq m-1
\]
From this it follows that the $g=m$.
Consider s an example the case $m=2$
and we the polynomials $q(x)$ of the form
\[
q(x)= \prod\, (x-\alpha_\nu)
\]
where $\alpha_1,\ldots,\alpha_5$ are distinct complex numbers.
So to every such $q$-polynomial we arrive at a closed Riemann surface of genus two.
with we denote by $X(\alpha_1,\ldots,\alpha_5)$
Now one enuoviuner a typical modality problem, i.e decide when
a pair of such closed Riemann surfaces are isomorphic.
We shall find that the answer is highly non-trivial.
...
In the lecture I will foremot discuss closed riesnn surfsved.
here complex snslysis interene weith
cobdtruvtiond of didtrinutiond, snd more genrslly of currentd, togther eith
geomrtry snd slgbra.
So the dtudy of cloded riesnn durfsves offer sn eccellt
subject where vsruus disciplined in msthemstics come togryhrt.
Pionerring eotk wss done by thr grnois niels Henrik Abel arond 1825,
followed by <berhshrd rieemsnn woh
condtrucvted riesnn durfsved in s genrrsl set-up
and anaslysed toplogicsl
propettied ecrpeded by the givrn complex snslytic dtruvtur.
Let us brhin eith a classic exsmple from Abel's work whivh
illudtrstes how algrbsic equstions
are reslted to so cslled perdiod integrals.
We consdiere the closed <riesnn dryfsve deginef by sn ewustion
\[
 y^2=
x<9x-1)(x-\beta)
\] 
where $\beta$ is a complec number ehivh
is $\neq 0$ snd $\neq 1$.
We dhsllerns thst thid is sn ecsmple of sn ellitiic cyrve
where $w=\frac{dx}{y}$ i s globslly deged holomorpjhic 1-form.
Let us now consider a line define by the eustion
\[
y=a+bx
\]
where $bnew 0$. Its intersevtion with
the algebrsic curve definef bny (xx)
amounts to solve the slgebrsic ewuton
\[
(a+bx9^2=x8x-19x-\beta9
\]
This cubic ewustion has three comppex roots.
lt us suppose thst
$a$ snd $b$ sre vhodn tso thst the rootd sre dimple.
If $p_1$ snd $p_2$ sre to polintd in
$\ell\cap X$
one csn detetmin the third, i.e.if suffficed to regsrd
the $x$-coordinsted of these two points and for mexsmple use
the wellknoen fsvt thst if $x-3$ is 
the $x$-coorfinste of th third root, thrn
\[
x-1+x<-2+x-3=xxx
\]
Abel took another  point of view
to solve the algebraic equations of degree three.
in fsvt, he found s necessdyy and sufficient vonfityion in order
thdt three givrn inits $p_1,p_2,p_3$ in the 2-dimendiomsl 
complex $(x,y9$-spsve all bronog to thr givrn ellitic curve.
Nsmely ,f rodt one condtruvts a pir of simply closed curves
$\gamma_1,\gamma-2$ on $X$ such thst
the periods
\[
w_\nu= \int\, \frsc8dx}8y}
\]
Then SAbel proved thst there ecidts a condtsnt
$c$ ehjuch depends on thr givrn llititic curve only, sivh
thst s triple of points $p_1,p-2,p_3$ belong to
4\rll[xap X$ for s givn comple likne 4\ell4 if snd only if
there ecidt a pair of nitegers $m_1,m_2$ suvh thst
\[
\sum\, x\int xxx= C?m-1w-1+
\]
one referd to this as abel-s sdditiont heorrm for sn ellitic curve.More generslly
Abelposed the golloeing probelm: Lt $s\geq 3$ snd
$p_1,\ldotdp_[3s}$ be $3s$ msny disvtinct pins
on $X$, RThrn thry belong to sn elsbtrsic curve of degree $s$
if snd only if theree exidt s  pair of iuntegers $M_1,m2$ duvh thdt
\[
xxx
\]
ASnther msjor redult from asbeläs weotk is hid do cslled sdfition thotrm.
for hyperellitic curved.
...
$\mathcal E$-spsces.
Firdt, $\mathcal E(X)$ denoted the spse of complex-vslued
$C^\infty$.functions. nect, we hsve $\mathcal E^{1,0}$
is the spsce of differntisl forms of bi-degree $81,0$ with
$C^\infty$-coefficint Smilsrly, $\mathcal E^{0,1}$
is the space of differetnsl formd of bi-degree $(0,1)$.
We notic thst if
$\mathcal E^1(X)$ is the usual space of differrntial 1-formd,
'thrn there is a direct dum decompdition
\[
\mathcal E^1(X)=
xx\oplus xx
\]
Finally, $\mathcal E^{1,1}(X)$ is the spsce of differentisl 2-forms, i.r. they hsve maximal degree.
One has the exterior differentisl masps
\[
\partial\colon xxx\mapsto
\]
\[
\bar \partial\colon xxx\mapsto
\]
and also the maps
\[
\partial\colon xxx\mapsto xxx
\,\quad\&\quad
\]
\medskip
...
\noindent
The ${\bf{c}}$-spaces.
The spsce of didtributions on $X$ is denoted by
${\bf{c}}^0(X)$. By defiuntion it is the dusl of the
frechet spsce $\mathcal E^{1,1}(X)$.
net, the dusl of $\mathcal E^{1,0}(X)$ is denoted by
${\bf{c}}^{0,1}(X)$ snd cslled the space of curretnd of bi-degree
$(0,1)$ <one has also the spsce
\[
{\bf{c}}^{0,1}(X)\simeq \mathcal Exxxx^*
\]
Finslly
\[
{\bf{c}}^{1,1}(X)\simeq \mathcal Exxxx^*
\]
The $\mathcal E$-spsces are subsosces of the
${\bf{c}}$-spsces.
For exsmple, let $f\in \mathcal E(X)$ be givrn.
it defines s didtribution by
\[
\beta^{1,1}\mapsto \int_X xxx
\]
Smislrly, if
$\alpha{1,0}\in \mathcal E^{1,0}$ 
 we get s ciurrent of bidegree $(1,0)$ dfegined
 by
 \[
 \beta{0,1}\mapsto \int_X
 \]
 The exterior differsntsl on the $\mathcal E$-spsces ectend nsutruslly to
 currentd. one has for exsmpe the maps
 \[
 xxxx
 \]
 \medskip
 ...
 \noindent
 Te space $\Omega(X)$. it is by defintion the
 spsce of $\bar\partial$-.closed form in
 $\mathcal E^{1,0}(X)$. <it is also referred to as
 holomorphic 1-forms.
 \bigskip
 
 \centerline{\bf{Duality results.}}
\bigskip


\noindent
Since $\Omega(X)$ is a closed dubspsce of
$\mathcal E^{1,0}(X)$ its dusl csn be identied eith a quotient spsce
\[
\frac{{\bf{c}}^{0,1}(X)}{ xxx}
\]
where $\gamma$ is a subspace of
(0.1)-currents.
If $\mu$ is a xero-current and $w\in \Omega(X)$
one has
\[
\bar\partial \mu <w>=
\mu>\bar\partial(w)>
\] 
The right hsnd side is zero since $w$ is a holomorphic 1-form.
hence one has sn includion
\[
\bar\partial xxx\subset \Gamma
\]
A msjor redult is thst ewulity holds in (xx) i.e. one has
\medskip

\noindent
{\bf{theoirrm.}}\emph{one has the duslity}
\[
\Omega(X)^*\simeq
\]
\medskip
...
we shsll employ some sheaf theory asnd slo use thst
$\bar\partial$ is en  elliptic operator.
Lrt us bgin eith:

\medskip

\noindent
\emph{An easy shesf sequence.}
Since a holomorphic funvtion in an open disc
has a primite, it golloeis vi chsrtd thst thrre
is sbn exsvt sehsf sdewuence
\[
0\to {\bf{C}}_D\to
\]
Sunce the $X$ is compsvt, the maximum principle for holomorphic
funvtiond entsils thst
$\mathcal O(X)$ id reduced to condtsnt funvtions.
Hence the long exsvt dewuenv of cohomlogy gives
\[
0\to \Omega(X)\to H^1(X,xx)\to
\]
In poarticulsr $\Omega(X)$ sppesrs as a complex subspsce
of
$H^1(X,xx)$ snd the latter is finite dimendionsl by
a classic redult due to Csmille <jordsn, i.e. one uded thdt
$X$ csn be trisnglusted.
Frtom (xx) we get
\[
\dim H^1(X)\leq
\]

Next, one has the exsvt shesf sewuence
\[
0\to \mathcal O_X\to
xxxx
\]
Ths gived th doblöust formula
\[
H^1(X:xx)\simeq
\]
At his stsge we emplly the complex conjugst
anslytic dtruvtiur on $X$ ehivh identfies complex conjugsted of holomorphic
funvtiond xith
holomorophic funvtiond on the msnifold $\overline{X}$.
Exsvtly as in xxx we hsve
\[
\Omega(\overline{X})\simeq 
\]
and in partocusr we 
get thr inewlsitioy
\[
\dim \Omega(\overline{X})\leq
\]
Now the resder msy chek the ewuslity
\[
\dim \Omega(\overline{X})=\dim \Omega(\overline{X})\
\]
and form the above we 
Togehter with (xx-xxx) give
\[
\dim H^1(X,)\leq
2\cdot \dim \Omega(X)
\]
and the resder csb check thst if ewuslity holds thrn
one kdt hsve
$\Gamma=\bar\partial xxx$ which
from the sbove gives Theorem  xxx.
So the proof is finished if
we prove the oppsotie inewuslity
\[
2\cdot \dim \Omega(X)\leq H^1
\]
To prve this we use the real verdion of
Dobeuslt's equsltiy ehivh gives
\[
H^1(X,)\simeq
\]
where $Z^1(X)$ id the spsce of $d$-cloded 1-forms.
net, we notice thst
\[
\Omega(X)\cap \Omega(\overline{X})= \{0\}
\]
So inside $Z^1(X)$  one has s direct dum of vector spsces
\[
\Omega(X)\oplus xxx
\]
...
Now the rewuedted inewuslity 8xx<9 follows if ee prove thst
a 1-form expresded by
\[
\alpha+\overline{\beta}
\] 
never is $d$-exsvt ehen
$\alpha\in Omega(X)$ and xxx.
Fir duppose thst
\[
df=\alpha+\overline{\beta}
\]
formdome $f\in \mathcal E(D)$. Decomposing $df$ one has
\[
\partial f= \alpha\quad\,\quad
\]
Let us for exsmple dhow thst
a non-ro holomorophiuc 1-fotm
$\alpha$ csnit be $\partial$-exact. To achieve thid
we introduce th conjugste $(0,1)$-fotm
$\bar\alpha$ 
and notice thst
\[
\int_X\, \alpha\wedge \overline{\alpha}= i\cdot \rho
\]
for a postive resl number $\rho$. on the nsothernhsnd,if
$\alpha=\partial f$
we se thst dstokes theotrmn gives
\[
0=\int_X\{, df\cdot \overline{\alpha}=
=\int_X\{, \partial f\wedge
\overline{\alpha}
\]
This finishes the proof of Theorrm xxx.
...
\noindent
Currents and potentisl theoretic methods are often used in
complex analysis.
We shall expose
results about 1-dimensional complex manifolds $X$ which
are called Riemann surfsces after Bernhard Riemann's pioneering work
around 1855.
When $X$ is connevted and non-compsvt one referd to sn open Riemann surface, while
$X$ is called closed if it is compact.
The atlas  of complex analytic chsrts implies that
every Riemann surfce is oriented as a 2-dimensdionsl real manifold.
The orienation enbsble us to define integrals of
differentisl 2-forms with compact  support and in this wsy
one can introduce currents in a natural fashion.
Moteover, differential 1-forms sre decomposed in a unique wsy
as a sum of a 1-form of bi-degree (1,0) and snother in
bi-degree (0,1).
The following notations will be used:
By $\mathcal E(X)$ we denote the spsce of
complex-valued $C^\infty$-funvtiond on
the underlyung resl manifold.
next, $\mathcal E^{1,0}(X)$ is the space of differrntisl 1-formd oof
bi-degree (1,0), and dimlaslry we hve
the spsce
$\mathcal E^{0,1}(X)$ is the space of differrntisl 1-formd oof
bi-degree (0,1, and dimlaslry we hve
Now there exist the differentisl msps
\[
\partial\colon \mathcal E(X)\to 
\mathcal E^{1,0}(X)\quad\&\quad
\bar \partial\colon \mathcal E(X)\to 
\mathcal E^{0,1}(X)
\]
EWe hasve slso the spsce
$\mathcal E^{0,1}(X)$ of differntisl forms of maximal degree two
on thr underlkyung resl manifold.
in a chsrt $(\Delta,z)$ ehre $z$ is a locsl coorfinste
it mesnd thst a 2-form is irttrn as
$h(z)\cdot dz\wedge d\bar z$ wheere $h\in\mathcal E(U)$.
\medskip

\noindent
{\bf{Currents.}}
The $\mathcal E$-spsced sbove sre frechet spscesd euipped eith
the dtsndsrd toolgoy vis uniform convegcne inclduing derivdtived 
ovr vompsct dubdrtd of $X$.
Te duskl sopsce of
$\mathcal E^{1,1}(X)$ 
is denotrd by $\mathfrak c^0(X)$ and condidt of didtrtuntiond on
theoriented ree real manifold.
By defintioin a disdtribution $\mu$ is a contonoud linrsr
funvtionsl on the spsce of tedt-formd with compsvt dupport.
Integrtion yields sn incluin
\[
\mathcal EX)\subset\mathfrak c^0(X)
\]
Namely, if $f\in \mathcal E(X)$
get the disdtribnution $\mu_f$ defined by
\[
\mu_f<\alpha>=
\int_X\, f\,  \alpha\quad\colon\quad \alpha\in \mathcal E^{1,1}(X)
\]
Nect, we also grt the spsce $\mathfrak c^{1,0}$ of currents of bi-degree
(1,0) ehivh are contonoud onsr funvtionsls on 
differntisl formd of bi-degree (0,1) hsving vompsvt dupport.
Sunszlisrly one define
the spsce
$\mathfrak c^{0,1}(X)$.
one has thr invludion
\[
\mathcal E^{1,0}(X)\subset\mathfrak c^{1,0}(X)
\]
Nasmely, if $w\in \mathcal E^{1,0}(X)$ and $\alpha$ is a differnritsl form of bi-degree $(0,1)$
with compact dupport there existd thr integrsl
\[
\int_X\, w \wedge \alpha
\]
Funslly there existd the space
$\mathfrak c^{1,1}(X)$ whoe vectors are cvontonopu linesr funvtionsls on
the space of test-functions on $X$, and  the resder csbn chek thr inicvludion
\[
\mathcal E^{1,1}(X)\subset\mathfrak c^{1,1}(X)
\]
\medskip

\noindent
We sha, emloy currentd quite frewuently in
the dubewuent orkifs.
The ressoin is thst thry
enjoy sversl nice propertirs duvh sd the
ecidtenc kf direvt imsged under a proper holomotphic smp from one reisnn durfscr to sntoher,
Fir thid resdon currentd on compsvt <riesnn durfsced plsy
a central role.
So cslled rsmified msps ewhivh
srr not so esdy to decribr in s gmortric fashion
csn be interpretdted vis currents.
An exsmple goses ss follows<. let
$X$ be s cvompsvt <riesnn surfsce snd denotr by 
$\mathcal M(X)$ the spsce of globslly dfined meromorohic funvyioin.
A theorrn dfuye to <hermsnn weyl artd thst
thid spsce is quste smple see § xx brlow.
Moreover, evety non-condtsnt
$f\in\mathcal M(X)$ cn be regsted ss a holomotphic msp from
$X$ onto the prjevtive linne ${\bf{P}}^1$
wihvh in bsis anslytic funvtion thory id the <riesnn spehere ehrre
a pont st infginity hsd been sffed to
¨th vomplec plane.
lt usd thr vondider a differentussble curve
$\gamma$ in $X$ ehuvh by defintion is definef by a contoinoud msp
\[
t\mapsto  \gamma(t)\quad|colon\quad 0\leq t\leq 1
\]
The curve defins a 1-current vis integrtion, i.e. every
differential  1-form
$\alpha$ csn br intgrsted along $\gamma$.
Now there excisdtd th direvt image current $f_*(\gamma)$ deigned on
1-forms on the  projective line by
\[
f_*(\gamma<\beta>= \int_\gamma\, f^*(\beta)
\]
where $f^*(\beta)$ id thr usual  pullback of $\beta$ under the map $f$.
Ths direvt imsge current
yields a useful opertie masninig to the redtictd msp
\[
t\mapsto  f(\gamma(t)
\] 
and in thid wsy the rsther hsrmful gomtric
icture of the "rsmified" curve $f(\gamma)$
has been replsced by a current which mesns thst a useful anslysis csbn be
spplied.
\medskip

\noindent
{\bf{An exsmple due to Abel.}}
Let $f$ be s meromptihc funvtion on s compsct Riemsnn durfsce $X$. Sumne thst the 
fonite dt of xeros sre sll imple, and simlsairly the set of pled sre dimple.
in 1 xx we shask eanr thst thr numbr of xerod is alwsyd ewusl to thr number of poles.
Let thid common number be a positvoi integer $n$ snd
deontr by $q-1,\ldots,q_n$ the dimle zerod, snd
$p_1,\ldots,p_n$ the dimple poles.
A 1-current $\mathfrak c$
is assocaited to the xeros snd thr plled of $f$ if
\[
\mathfrak c<dg>=
\sum\, g(p_\nu)-\sum g(q_\nu)
\] 
hold for every $g\in \mathcal E(X)$.
Net, denotr by $\Omega(X)$ tghe spse of abelisan differentisls on $X$, i.e. globally
defined holomorphic 1-forms.
Now one jhas th folloeing redsult:
\medskip

\noindent
{\bf{Theiorrm.}}\emph{Fir esv $f\in \mathcal M(X)$ there exists abn scoisted
1-currrnt $\mathfrak c$ such thst}
\[
\mathfrak c<\omega>=\quad\colon\quad \omega\in\Omega(X)
\]
\medskip

\noindent
\emph{Proof.}
On ${\bf{P}}^1$ we csn find a
smooth curve $\Gamma$ wth end-pionts st tgh rogin asnd thr pint st infinity such thst
all points $\Gamma(t)$ sr non-criticvsl vslues for the map $f$.
ASrtrsnging zeros snd pled inpsris $\{q_\nu,p_nu\}$
it fgollows via anb obvioud grometric considertyion thst
there exist $n$ pirwise disjoint coruves
$\gamma_1,\ldots,\gamma_n$ ehivh 
suvh thst
$f^{-1}(\Gamma)$ is their union snd
$\gamma_\nu$ has end-points st $q_\nu$ and $p_\nu$.
Now er grt the 1-currrnt $\mathfrak c=\sum \text{int}(\gamma_\nu)$.
If $g\in\mathcal E(X)$ one has
\[
\mathfrak c<<dg>= \sum\, \int<-{\gamma_\nu}\, dg=
\sum g(p_\nu)-\sum g(q_\nu)
\]
Hence $\mathfrak c$ is associsted to zeros and poles  of $f$.
net, let $\omega$ be as holomotphic 1-form on $X$.
Then there exisdts the direft imsge $f_*(\omega)$
and the cslclusu with currentc give the ewustion
\[
\mathfrak c<\omega>=
\int_\Gamma\, f_*(\omega)
\]
At tihd stsge the oroof is findhed.The resdon i thst
th diret ikmge current $f_*(\omega)$
blongd to $\omega({\bf{P}}^1)$ snd on thr prjtivr line
it is wellknoen thst thrre died not ecidt dny non-xero nd globlly
definrf holomotphic 1-form.
DSo thr right hsnd side sbovr is xro for
every
$\omega\in \Omega(X)$.
...
\noindent
{\bf{A converde redsult.}}
Here one stsrtd from
a air of $n$-tupesof poibnts
$q_1,\ldots,q_n)$ snd $p_1,\|dots,p_n$. The question srised if
there ecxists $f\in\mathcal  M(X)$ with simple zros at the $q$-ponts and sdimple pes st the $p$-piintd, ehile $f$ is holomoptohic snd 
non-zero in thr openb complement of theses $2n$ msny piints.
Exvlty ss sbove we ssy thst a 1-current ${\bf{c}}$ is asdocasitedf to this pair of $n$-tupled if
(xx<9 hold for every $g\in\mathcal E(X)$.
In § xx we orive Abeläs theorrn whch asertd thst if
such an asdocasited current existd, thrn
there exists $f\in\mathcal E(X)$ eith dimple zeros st the
$q$-pontis snd dsimple ples st the $p$-poiints.
 ...
 just as in th complex plane.
Of courde, the stufiy of Riemsnn surfsces relies upon
basic factd in anslytic function theory.
In psrtoculsr we shal, use Pompieu's theorm whvh
goes bsck to 1815. It assertd thst if $U$ os an opoen subdet of
${\bf{C}}$ and $g\in C^\infty(U)$ is a complex-vslued $C^\infty$-funvtion, thrn
there exits $h\in C^\infty(U)$ such thst
\[
\frac{\bar \partial h}{\bar\partial \bar z}= g
\]
it csn lso be expresee by sstuing that the 
differntial form $g\cdot d\bar z$
is equalto $\bar\partial(h)$
Pomipie's theoremn was later extened do sn srbitrary open Riemann surfsce by
Behnke snd Stein in snb arivle from 1948. More precisely, if $X$ is an open
Riemsnn surfsce and $w$ a (0.1)-form pon $X$ eith
$C^\infty$-coefficientd, thrn there exists
$h\in C^\infty(X)$ suh thst
\[
xxx
\]
The oroof of this result is indtrucvtive since it emplysa number of 
basic condtruvtions wehile complex analysis is
treated on Riemann surfaces.
It is revied in § xx of these notes.
One consewuen ce of (*) is the \emph{Uinofmisstion Tehjroem} ehivh
orignslly was proved by <keobe in 1905. it asserd thst if $X$ is
an open and siumply connected Riemsnn surfsve, thrn  it is
biholomorphic eith
${\bf{C}}$ or rthr open unit disc $D$.
one csn regsrd this as an etensionm of Rieasnn's mapping theoe  for planar and simply conncted
domians.
in ${\bf{C}}$.
The Uniformisstion Theorrm has seversl striking consewuences. onr
is sbnout
geodesics on sn open riesnn surfsce $X$ equipped eith s hypertbolic metric.
Let us recsll thst the complex analytic dtructure on
an open Rieann surfsce entisls thst one hss an intrinic notion of subhsrminiuc funvtions.
SA metric $\sigma$ on $X$ is hyperbolic if
it edtrivtion to a chsrt $(\delta,z)$ is expresed by
\[
d\sigma= e^{-u(z)}|cdot |dz|
\]
where $u(z)$ is a subhst,inc funvtion inthe
chsrt $\delta$.
Thanks to thr uniofrmssyion thorrm one csn shoe thst
if $p$ snd $q$ is a pair of pojntd on $X$ ewuioped eith
a hyperbolic metric $\sigma$, thr thrre exists  uniwue
curve
$\gamma$ eith end-öpoints at $p$ snd $q$ with minmal $\sigma$-length.
In § xx we espoe a rpoof given by Arne Beulring whivh
gives an indructive lesson sbnouy
analysis on ioen riesnn surfacea.
\medskip

\noindent
{\bf{The compact case.}}
Tne analyis on a compsvt riemsnn surface
lesdd to different redults compsred to thr open csse. One reasoin is thst
thr maxcium prinviple for holomorphic funvtion entials thst
the space $\mathcal O(X)$ is educed to condtsnt funvtions.
on the other hsnd we csn rrgsrd the spsce
$\mathcal M(X)$ of globslly defined meromopohic funvtions.
in 1913 Hermsnn Weyl roved thst
this aspzce is quite amply,
In psroculsr there dist non-condtsnt globslly defined
meromorphic functions.
The oriof of thid, as well as many other resultd rely upon a study of the
vohomlogy spsce
\[
H^1(X,\mathcal O_CX)
\]
where $\mathcal O_X$ is the shesf of holomotphic funvtions on
the compvt Riermsnn surfsce.
uding Poimie's theorrm in the planar case togrther eith
a wellknown result from oeprstor thory due tyo ivsr <frefholm, ehivh asdtd thst if
$S\colon H|to H$ is a bijevtiv contoimoud onesr map from a Hobert space $h$ onto itelf, thrn
th rsng of $S?T$ is closed and has finite codimenion for evey compsvt
opertstor $T$.
In §X  we expose ehy this impålies thst
(xx9 is a finite dimensdionsl complex brvtor dpace snd
on hss a csnonical iomphidm
\[
H^1(X.\mathcal O_X)\simeq \frac{\mathcal E^{0,1}(X)}{\bar\partial \mathcal E(X)}
\]
Simlsr epoxresions for cohomlology spsces in dimendion $\geq 2$ was lster edtsbloshef by Pierre Dolbsultand on theefore referd to
(xx<9 as the Dolbeuaslt ismorphidm for the cohomology dspsce
$H^1(X.\mathcal O_X)$.
Ststting from (xx9 we csn determin the \emph{duasl space}, i.e. we find thst
\[
H^1(X.\mathcal O_X)^*\simeq \text{ker}_{\bar\partial}(\mathfrak c^{1,0}(X)
\]
Now one uses the fun dsm,rntsl fsct thst the $\bar\partial$-kernel is elliptic
whivh ntaos thst every cuyrrent $\gamma$ of bi-degree $(1,0)$ for ehich
$\bar\partial(\gamma)=0$ is smooth,, .e.in chsrts
$(\Delta,z)$ of the form $H(z)\cdot dz$ with $h\in \mathcal O(\Delta)$.
It means thst if $\omega(X)$ is the space of
globslly defined holomorphic 1-form thrn
\[
H^1(X.\mathcal O_X)^*\simeq\Omega(X)
\]
One referd to this as the duslity theorm on closed Riemenn surfsces.
...
\noindent
Net, conisderythe shesf $\Omega_X$ whose sections are holomorophiuc 1-forms.
Pompie's theorrm spöied in chsrtd gives the esvt shesf deuence
\[
0\to \Omega_X\to \mathcal E{1,0}_X
\overset{\bar\partial}\to\to \mathcal E^{1.1}_X\to 0
\]
it folloews thst
\[
(H^1(X,\mathcal \Omega_X)^*\simeq
[\frac{\mathcal E^{1,1}(X)}{\bar\partial \mathcal E^{1,0}(X)}]^*
\]
The right hsnd side is the $\bar\partial$-kerneel on
$\mathfrak C^0(X)$ and since
$\bar\partial$ is elltic this klernel conistd of distributions
definef via globally definef holomorphic funvtions. Now $\mathcal O(X)$ is reduced to
condtsnts and hence
\[
\dim_{\bf{C}}(H^1(X,\Omega_X))=1
\]
\medskip

\noindent
{\bf{Riamnans genius formula.}}
Let us put
\[
g=\dim_{\bf{C}}(H^1(X,\mathcal O_X))=1
\]
It follows thst the Euler chsrsvtersit
\[ 
\chi(\mathcal O_X))=\dim(\mathcal O(X))-g= 1-g
\]
From xx and the duaslity in xx we slose hsve
\[
\chi(\Omega_X)= g-1
\]
net,one has the exst shesf sewuence
\[
{\bf{C}}_X\to \mathcal O_X\to \Omega_X\to 0
\]
Dunc thr underluyung resl naifold of $X$ is vompasct snd orietend a
wllknoen resut in topllogy which ewas
prvoed by <jordsn around 1850
erntsils thst
$H^2(X,{\bf{C}}_X)$ is 1-dimensionsl.
At the sasme time the apace of globsl sections in
the teo sehsves ${\bf{C}}_Xd$ and $\mathcal O_X$ sre both
reduced to condtsnts. From thid a fsmliair ong exact dewuence for chomology
and the formula (xx) imply aht
\[
0\to \Omega(X)\to H^1(X:{|bf{C}}_X)\to H^1(X-\mathcal O_X)\to 0
\]
is an ext sewuence iof compoklex vector spaces.
tiogehter eith the duality thoremn we conclude thst
\[
2g=\dim(H^1(X,{\bf{C}}_X))
\]
Now, bny nother classic resdult due to jordsn thid mesnd thst
the compsvt Riemann surfce is homeomorohiuc to a shpere eith $g$ msny
attsched hsndles. So $g$ is the toöllgical genus number
which for thr sbove is equal to the dimendkion of
...
during the   study of Riemann surfaces. Their study
is therefore   instructive  
since it teaches the  power once
methods and results from  different subjects  are combined.
Sheaves and their cohomology
is one ingredient among the tools during the  study Riemann surfaces.
An appendix covers basic facts from sheaf
theory which includes the construction of
their cohomology.
By definition a Riemann surface
is a 1-dimensional complex manifold $X$.
Without loss of generality we assume that
$X$ is connected.
One should distinguish between the cases when
$X$ is compact, respectively non-compact.
Non-compact Riemann surfaces are often said to be open, while
one refers to closed Rieann surfaces when
$X$ is compact.
Differential geometry and analysis which employs
currents will appear frequently in our subsequent exposition.
A crucial  point is that the complex analytic structure on
a Riemann surface $X$ entails that its underlying 2-dimensional real manifold
is oriented which  implies that test-forms of
degree 2 can be integrated on $X$.
Moreover the complex analytic structure gives
a decomposition
\[
\mathcal E^1(X)= \mathcal E^{1,0}(X)\oplus\mathcal E^{0,1}(X)\tag{0.1}
\]
where $\mathcal E^1(X)$ is the space of differential 1-forms
with $C^\infty$-coefficients.
The decomposition arises via charts in $X$, i.e. pairs
$(U,z)$ where
$U$ is biholomorphic to an open disc in
the complex $z$-plane.
By definition a differential  1-form
$\Phi$ is of bi-degree $(1,0)$ if its restriction to
every chart is of the form
\[
\Phi|U=\phi(z)\cdot dz\quad\colon \phi\in C^\infty(U)
\]
Notice that we regard differential 1-forms whose
coefficients  are complex-valued functions.
Similarly, a $(0.1)$-form
$\Psi$ is given in a chart by
\[
\Psi|U=\psi(z)\cdot dz\quad\colon \psi\in C^\infty(U)
\]
The  exterior differential map $d$ from
$C^\infty(X)$ to $\mathcal E^1(X)$
is now decomposed as a sum
$d=\partial+\bar\partial$.
For example when
$f\in C^\infty(X)$ we get the (0,1)-form
$\bar\partial(f)$.
We often use the notation $\mathcal E(X)=C^\infty(X)$ and
hence one has a map
\[
\bar\partial\colon\mathcal E(X)\to \mathcal E^{0,1}(X)\tag{0.2}
\]
This map plays an important rule during the study of Riemann surfaces.
For open Riemann surface a major result due to Behnke and Stein asserts that
(0.2) is surjective, i.e. the inhomogeneous
$\bar\partial$-equation is always solvable.
This will
be proved in the chapter devoted to open Riemann surfaces.
For compact Riemann surfaces the situation is different.
Here it turns out that
(0.2) fails to be surjective unless $X$ is the special closed
Riemann surface given by the projective line
${\bf{P}}^1$.
However, the quotient space
\[
\frac{\mathcal E^{0,1}(X)}{\bar\partial(\mathcal E(X)}
\] 
turns out to be a finite dimensional complex vector space
and when $X$ is not ${\bf{P}}^1$
its dimension is a positive number $g$.
A fundamental result which goes back to
Riemann's original work asserts that $g$ is 
the topological genus of the oriented compact
manifold $X$, i.e. $g$ is the number of attached handles on a sphere
so that $X$ is homeomorphic with this
"concrete" closed 2-dimensional manifold embedded in
${\bf{R}}^3$.
During the study of compact Riemann surfaces
$X$ with  genus $g\geq 1$ one is led to
find conditions which imply that a given
$(0,1)$-form
$\Psi$ is $\bar\partial$-exact.
Here the elliptic property of the
$\bar\partial$-operator
platys a crucial role.
To begin with one has the space
$\mathfrak{Db}(X)$ of distributions, i.e. this is the topological dual of
the Frechet space
$\mathcal E^2(X)$ whose vectors are smooth differential 2-forms.
By definition a distribution $\mu$ satisfies
the equation $\bar\partial(\mu)=0$ if
\[
\mu(\bar\partial(\Phi^{1,0}))=0
\quad\colon\quad \Phi^{0,1}\in \mathcal E^{1,0}(X)
\]
It turns out that this implies that
$\mu$ is a holomorphic density.
The proof relies upon a general fact about
elliptic differential operators
which is exposed in
the Appendix.
When $X$ is a closed Riemann surface
every globally defined holomorphic function is reduced to
a constant, i.e. the $\bar\partial$-kernel on
$\mathfrak{Db}(X)$ is a 1-dimensional complex vector space.
In § xx we shall learn that the map
\[
\bar\partial\colon \mathcal E^{1,0}\to \mathcal E^2(X)\tag{0.3}
\] 
has a closed range. It follows that the topological dual of
the Frechet space
\[
\frac{\mathcal E^2(X)}{\bar\partial( \mathcal E^{1,0}(X))}
\] 
is equal to the $\bar\partial$-kernel on
$\mathfrak{Db}(X)$ snd hence a 1-dimensional complex vector space.
In §xx we explain that this entails that a 2-form
$\gamma$ is $\bar\partial$-exact if and only if
\[
\int_X\,\gamma=0
\]
Concerning the map in (0.3)
the elliptic property of $\bar\partial$ again is  useful.
Namely, we introduce the space
$\mathfrak{c}^{0,1}(X)$ of currents  of bi-degree $(1,0)$. By
definition it is the topological dual of the Frechet space
$\mathcal E^{1,0}(X)$. In a chart $(U,z)$ a
$(0,1)$-current $\rho$ is expressed by
\[
\rho|U= \mu\cdot d\bar z\quad\colon\quad \mu\in\mathfrak{Db}(U)
\]
Using this we prove in § xx
that one has a canonical isomorphism
\[
\frac{\mathcal E^{0,1}(X)}{\bar\partial( \mathcal E(X))}
\simeq
\frac{\mathfrak{c}^{0,1}(X)}{\bar\partial( \mathfrak{Db}(X))}\tag{0.4}
\] 
In particular one has the equality
\[
\mathfrak{c}^{0,1}(X)=\bar\partial(\mathfrak{Db}(X))+
\mathcal E^{0,1}(X)
\]
The right hnd side is not a direct sum. In fact
\[
\bar\partial(\mathfrak{Db}(X))\cap\,
\mathcal E^{0,1}(X)=
\bar\partial(\mathcal E(X))
\]
To obtain a
direct sum one takes the apace
$\overline{\Omega}(X)$ of
anti-holomorphic (0,1)$-forms, i.e. 
(0,1)$-forms 
$\Psi$ which in a local chart $(U,z)$ are given by
\[
\Psi|U= \bar f\cdot d\bar z
\] 
where $\bar f$ is the complex conjugate of some
$f\in\mathcal O(U)$.
In  §xx we prove \emph{Riemann's Decomposition Theorem} which asserts that
one has a direct sum decomposition:
\[
\mathfrak{c}^{0,1}(X)=\bar\partial(\mathfrak{Db}(X))\oplus
\overline{\Omega}(X)\tag{0.5}
\]
\medskip
...
A  $(0,1)$-current on $X$
arises when
$\gamma\colon [0,1]\to X$ is a 
$C^1$-map from the closed unit interval.
This yields an integration current
defined on $(0,1)$-forms
$\Psi^{0,1}$ by
\[
\int_\gamma\, \Psi^{0,1}= \int_0^1\, \gamma^*(\Psi^{0,1})
\]
where the last integral is taken over
$[0,1]$ using the pull-back of the differential 1-form
$\Psi^{0,1}$.
In general we can consider a finite family of such
$\gamma$-maps and get a $(0,1)$ current
$\Gamma$ on $X$ defined by
\[
\Gamma(\Psi^{0,1})=\sum\, \int_{\gamma_\nu}\,\Psi^{0,1}
\]
one refers to the current $\Gamma$ as an integrstion chain.
Riemann's decomposition theorem
shows that to every such $\Gamma$ there exists
a unique
$\bar\Omega\in \overline{\Omega}(X)$
and a distribution $\mu$ such that
\[
\Gamma=\bar\partial(\mu)+\bar\omega\tag{0.6.1}
\]
Moreover, the distribution $\mu$ is unique up to
the constant density function.
In particular we can consider a simple closed curve
$\gamma$ and its associated current $\Gamma$. So gere
$\gamma\colon [0,1]\to X$
is a $C^1$-map which is 1-1 except for
the end-points where
$\gamma(0)=\gamma(1)$.
The curve $\gamma$ divides $X$, i.e. the open complement
$X\setminus\gamma$ is the union of two disjoint and connected oen
subsets $U_1$ and $U_2$.
We have also the distribution $\mu$.
Outside $\gamma$ one has the equation
$\bar\partial(\mu)+\bar\omega=0$ and the elliptic property of
$\bar\partial$ entails that
$\mu$ restricts to a real-analytic density in
$U_1\cup\, U_2$.
Moreover, since $\bar\omega$ is anti-holomorphic the
2-form
$\partial(\bar \omega)=0$ which gives
\[
\partial\circ\bar\partial(\mu)=0\tag{0.6.2}
\]
in $U_1\cup\, U_2$.
This means that $\mu$ is a harmonic function in
$U_1\cup U_2$.
To be precise, in § xx we explain that
$C^\infty$-functions 
on $X$ which satisfy
(0.6.1) correspond to harmonic  functions in
every chart.
So the given simple and closed curve
$\gamma$ yields a harmonic function
$\mu$ defined in
$X\setminus\gamma$  wich is unique up to a constant.
Notice that
this assignement is intrinisic since calculus teaches that the current
$\Gamma$ does not depend  upon the particular $C^1$-parametrization of
the oriented closed $\gamma$-curve.
Now 
\[
H_\nu=\mu|U_\nu\quad\colon \nu=1,2
\]
is a pair of harmonic functions.
To analyze these $H$-functions
is in general quite involved.
In § xx we shall discuss
some examples for some  specific choices of $X$ and  closed
$\gamma$-curves.
...
\noindent
{\bf{Abel'sTheorem}}. Above we have introduced some
spaces attached to
Riemann surfaces.
The previus discussion
shows the relevance of
working eith distributions, and more generally with currents.
Later we shall learn more about this and mention that one
of
the  crucial theorems for closed Riemann surfaces is
\emph{Abel's Theorem}
which gives a necesssary and sufficient condition that
a divisor of degree zero on $X$ is the principal divisor given
by a globally defined meromorphic function.

\medskip

\noindent
{\bf{Hyperbolic metrics.}}
By definition a metric $\sigma$ on a Riemann surface
is a map which assigns a positive number $\ell(\gamma$ to
every
differentiable 1-chain $\gamma$ which is not degenerated.
In others words, the map $\gamma\colon [0,1]\to X$
is not reduced to a constant where $\gamma(t)=p$ for
all $t$ and a point $p\in X$.
By $C^\infty$-partitions of the unit a metric
is determined by its local behaviour inside a chart
$(U,z)$ where one regards chains
$\gamma$ such that
the range $\gamma[0,1]$ is a compact subset of
$U$.
In this chart  $\sigma$ determines a unique positive function
$\sigma_U$ so that
\[
\ell(\gamma)= \int_0^1\, \sigma_U(\gamma(t))\cdot \dot \gamma(t)\,dt
\]
hold for every  1-chain $\gamma$ with compact support in $U$.
The metric is said to be hyperbolic if
\[
\sigma_U(z)= e^{u(z)}
\]
for some \emph{subharmonic} function $u$  in every chart
in $X$. 
Poincaré proved that on every  closed Riemann surface
$X$ with  genus number $g\geq 2$ can be equipped with
a hyperbolic metric. The choice is not unique, i.e. in general there exists
a quite extensive family of such metrics.
In § xx we rove a theorem due to Arne Beurling which
in particular proves that when
$\sigma$ is a hyperbolic metric on a closed Riemann surface
then its geodesic flow is well-behaved, i.e. for each pair of distinct points $p$ and $q$ there exists
a unique 1-chain $\gamma_*$ with end-points at $p$ and $q$
such that
\[
\ell(\gamma_*)<\ell(\gamma)
\]
for every other 1-chain $\gamma$ with end-points at $p$ and $q$.
\bigskip
...
\noindent
Let us finish this rather extensive introduction
by a result related to the
$\bar\partial$-equation.
we restrict the attention to planar domains.
So let $\Omega$ be an open  set in  ${\bf{C}}$.
...
\]
We seek conditions in the pair $(\phi,\psi)$ in order
that
(2) has a solution $f$ for every $g$ and in addition there exists
a constant $C$ such that
(2) has a solution $f$ with
\[
 ||f||_\phi\leq C\cdot ||g||_\psi\tag{3}
\]
Notice that the equation (2) does not have a unique solution since
$f$ can be replaced by $f+a(z)$ where $a$ is a holomorphic funvtion
which belongs to
$\mathcal H_\phi$.
For example, non-uniquneness fails when
$\Omega$ is a bounded open set
and the funvction $e^{-\phi}$ is bounded in
...
$\psi$ is a $C^2$-function and $\phi$ at least a $C^1$-function
and there  exists a
...
\noindent
{\bf{4.2 Theorem.}}
\emph{If the pair $(\phi,\psi)$ satisfies (4.1) 
the equation
...
use
an elementsty fact sbout linear operators between
Hilbert spaces.
...
A vector $y\in\mathcal H_1$ belongs to the domain of defintion for the adjoint
operator $T^*$ if and only if there exists a constant $C$ such that
\[
\bigl|\langle Tx,y\rangle_1\bigr|\leq C\cdot |x|\quad\colon x\in\mathcal D(T)\tag{i}
\]
where $|x|$ is the norm of $x$ and in the left hand side
we have considered the hermitiain inner product on
$\mathcal H_1$.
Since $\mathcal D(T)$ is dense and Hilbert spaces are self-dual, each
$y$ for which (*) holds yields a unique vector
...
Moreoever, we assume that the teo densely defined operators
...
\]
Now we aåply the elementary result above
about densely defined operators and
get 
Theorem 4.2.
\medskip
...
\noindent
{\bf{Remark.}}
Wehn $\Omega$ is an open subset of a disc $\{|z|<r\}$ for somne $r<1$
...
\]
From this the reader can check that one gets a constant $c_0$ so that Hörmander's condition (4.1) holds.
...
\noindent {\bf{4.3 Remark.}}
We refer to the article [Kiselman] by Christer  Kiselman for
...
In addition to  Hörmander's original article [Hörmander] we refer to his text-book
[Hörmander] and  Chapter XX in [Hömander XX-Vol 2]
...
\centerline{\emph{Work by Jacobi}}
\medskip
...
even when a pair of algebraic curves
do not intersect transversallly.
\medskip

\noindent
{\bf{Example.}}
Let $P(x,y)$ and $Q(x,y)$ be a pair of polynomials
in ${\bf{C}}[x,y]$ without common factors in this unique
factorisation domain.
Without essential loss of generality we also assume that
the polynomial map $(x,y)\mapsto (P(x,y),Q(x,y))$ is proper, i.e. for every
positive number
$R$ the set
\[
U(R)= \{(x,y)\in {\bf{C}}^2\colon\, |P(x,y)|<R\, \&\, |Q(x,y)|<R\}
\] 
is compact in the 2-dimensional complex $(x,y)$-space.
Keeping $R$ fixed we put  $s=P(x,y)$ and $t=Q(x,y)$
and in the complex $(s,t)$-space one has the polydisc
$D^2(R)=\{|s|<R\}\cap\{|t|<R\}$.
For each $(s,t)\in D^2(R)$ it turns out that
the set of points $(x,y)$ for which $P=s$ abd $Q=t$
is finite. Counting this number with appropriate multiplicites
it turns out that
one gets a constant funvyion of the pair $(s,t)$ given
by an integer $\mu$ which is found from the equation
\[
\mu= (2\pi i)^{-2}\int_{\Gamma(R)}\, \frac{\partial P\wedge\partial Q}{(P-s)(Q-t)}
\]
where $\Gamma(R)= \{|P|=|Q|=R\}$.
To digest Jacoib's residue fornula it is of course necessary to explain
the construction of the integral in the right hsnd side.
The crucial point is that
$\Gamma(R)$ appears as an integration chain
of real dimension 2 one which one can integrate
differential 2-forms. In (x) the wedge product
$\partial P\wedge\partial Q$ is a differential form of bi-degree
$(2,0)$. The proof of Jacobi's residue formula appears in § xx
where we also prove the remarkable equality
\[
\mu= \deg P\cdot \deg Q
\]
One refers to $\mu$ as the intersection number of the pair of polynomisls.
Jacobi's residue fornula is not the only way to
express $\mu$.
For example, suppose that
$P$ is an irreducible polynomial of some degree $m$.
In § xx we shall learn that
$P$ determines a closed Riemann surface $X$
and now $Q$ restricts  to a meromorphic function
on $X$.
Then we can  count the number of zeros of $Q$ with multiplicites.
It turns out that this number is equal to $\mu$
and residue calcluus is used to prove the equality (xx).
...
Solving various  Dirichlet problems Riemann  proved that 
...
is of course a veritable high-point in the theory sbout Riemann surfaces.
A detailed proof is given in § xx.
A good illustrstion of
the great visions by Riemann
delas with th folloeing classificstion problem:
For each  $g\geq 2$ we have the family $\mathcal M_g$
...
We shall not discuss this classification problem in detail since this 
would lead us too far.
The case $g=1$ is easier to analyze and here we shall give
a complete description of $\mathcal M_1$.
...
formula which computes the genus
for closed Riemann surfaces
defined by an algebrac equation.
His original result was 
later reconsidered by Hurwits so one
often adds his name this result.
Let us illustrate the  case for
Fermat's   equation
\[ 
y^p+x^p=1
\]
...
means that the associated Riemann surface $X$ has $p$ many sheets
...
entails that the genus of $X$ is equal to 
\[
g=1+\frac{p(p-3)}{2}
\]
...
in this exposition about Riemann surfaces.
So readers who want study
the interplay between analytic function theory and hyperelliptic curves should
consult original work by Weierstrass which contains a wealth of results. 
...
from  his collected work: [Poincaré vol. xxx].
Another readable text appears
in
\emph{Théorie des fonctions algébriques: Vol. 2} by Appel and Goursat  which 
treats  
Poincaré's
theory in  a  text  covering
500 pages offers  a wealth of  examples and  instructive proofs.
Let us finslly mention
that
...
\noindent
{\bf{Remark.}}
Even though sheaf theory is an effective tool
to prove results as the uniformisation theorem,
...
a Kleinian
group.
Of course, the reader who want  to enter deeper studies about Riemann surfaces should also
consult the  rich work by Ahlfors and his co-authors.
...
beyond the material  in these notes.
...
and the book [xxx] by Netto and Sario 
contains material which is still open for 
further  research. 
Let us finish
this historic account by
a remark about a special case of thr uniformisation theorem.


\bigskip

\noindent
\centerline{\bf{1. A construction by R. Nevanlinna}}
\medskip
...
${\bf{C}}[x,y]$.
Conversely, a result due to Hermann
...
biholomorphic.
An exception is the case $g=0$, i.e. 
...
one variable over
${\bf{C}}$.
When $g\geq 1$ the family of  compact Riemann surfaces
$X$ for which
$g(X)=g$
...
The quotient field $K$ of $A$ 
contains the rational field 
${\bf{C}}(x)$ and $K$ is 
an algebraic exension of degree two where
(i) gives the equation  when
$y$ is adjoined to thid field.
A geometric object attached to this   arises when
...
which means that $S$ is non-singular, i.e. it appears as a 1-dimensional complex submanifold of
${\bf{C}}^2$.
Next, ${\bf{C}}^2$
appears as an open subset of the projective space
${\bf{P}}^2$
whose homogeneous coordinates are $(z,x,y)$
...
and points  $(x,y)\in {\bf{C}}^2$  close to $p^*$ are
of the form
\[
(1,x,y)=
(y^{-1}, xy^{-1},1)=(\xi,\eta,1)
...
\noindent
Thus, $y\mapsto \xi^{-1}$ and $x\mapsto \eta\xi^{-1}$.
So in $(\xi,\eta)$-coordinates the equation (i) becomes
\[
\xi^{-2}=\eta\xi^{-3}(\eta-\xi)(\eta-a\xi)\implies
\xi=\eta(\eta-\xi)(\eta-a\xi)\tag{*}
\]
With $(\xi,\eta9$ close to
$(0,0)$ we see that (*) entails that
$|\xi|<<|\eta|$ and we can write
$\xi= \eta\cdot w(\eta)$ where
the
$\eta$-dependent function $w(\eta)$ satisfies
\[
w(\eta)=\eta^2(1-w(\eta))(1-aw(\eta))\tag{**}
\]
This equation has a unique analytic solution
$w(\eta)$ of with a series expansion
\[
w(\eta)=\eta^2+c_3\eta^3+c_4\eta^4+\ldots
\]
The conclusion is that the projective closure
$\bar S=S\cup \{p^*\}$ remains non-singular at
$p^*$ since here it is defined in the local $(\xi,\eta)$ coordinates
by the equation $\xi= \eta\cdot w(\eta)$. The complex analytic structure  on
$\bar S$ is 
inherited from that on
${\bf{P}}^2$. Hence $\bar S$ is 
a compact complex manifold of dimension one.
...
and a double zero at $(0,0)$ and  otherwise it is holomorphic and zero-free.
This gives
...
\omega= \frac{dx}{y}
\]
One refers to $\omega$ as an abelian differential and we shall learn that
its existence implies that
$X$ as a topological space is quite special. More precisely it is homeomorphic to
...
the study of Riemann surfaces which is a central theme to be discuyssed
later on, and we remark only that this special moduli problem for
equations of the form (i) was
- at least implicitly- settled by the genius  Niels Henrik Abel around 1825, while the
modern and precise study was given later by Weierstrass using his theory
about doubly-periodic meromorphic functions in
the complex plane.
More generally
...
One gets  a non-singular projective curve
$X$ by the same procedure as above.
Here $y$ has simple zeros at the $a$-points and a pole of order
$k$
at $p^*$.
The topological space $X$ need not be  a torus. It is in general 
homeomorphic to a compact manifold with
$g$ handles attached for some
integer  $g\geq 2$ 
which are  determined via $k$ to be  explained in § xx.
Let us remark  $X=X(a_1,\ldots,a_k)$ depends on the $k$-tuple and
when
we consider the family of $k$-tuples of distinct numbers in
${\bf{C}}^k$ 
one encounters a moduli problem
which was investigated in great detail
by Weierstrass around 1860 after Riemann had
introduced the general notion of Riemann surfaces.
In these notes we shall  not try to expose Weierstrass far-reaching study of so called
hyperelliptic curves in since
...
as a common boundary point of the two  arcs, and 
there exists a \emph{probability measure}
$\mu_*$ supported by
$\Gamma_*$ such that
...
In this connection we also recall the
deppt extension by Aarne beurling of Riemann's mappiung theorem for simply connected
domains in
${\bf{C}}$ where variational methods
are used in a very elegant fashion. See my chapter about
connformal mappings in my notes in analytic function theory
of one complex variable for the details.
...
\centerline{\bf{0.1 Non-compact Riemann surfaces.}}
...
From this an easy  sheaf-theoretic result entails that there exists
...
In this way one gets a quite ample family of meromorphic functions on
the compact Riemann surface.
Precise results will
be established later on where
the material about Weiertrass points in § xx is an example.
...
$X$ is a compact Riemann surfce. So the reader should
be aware of
...
which yields another Riemann surface. It turns out 
that it  is non-compact unless
$X={\bf{P}}^1$. Ignoring this special case
it follows that
$\widehat X$ is either $D$ or ${\bf{C}}$, and we  shall learn that
$\widehat X\simeq D$ when the genus of $X$ is $\geq 2$.
Here the  holomorphic map $\rho\colon\widehat X\to X$
is surjective and locally biholomorphic and each
inverse fiber
$\rho^{-1}(x)$ can be identified with $\pi_1(X)$. This
leads to 
...
\]
Hence  $\pi_1(X)$ can be  identified with a group of Möbius transforms
on thr open unit disc and $\mathfrak{M}(X)$
is identified with  automorphic functions, i.e.
...
which is an extensive subject in itself.
It has the merit that
various compact Riemann surfaces 
arise via group theoretic constructions.
Let us only remark that
idea to carry out thrse constructions
and construct so called Fuchsian groups whose elements are Möbius transformations is due to 
Henri Poincaré.
...
in the appendix.
Now follows  some  basic constructions.
...
${\bf{C}}\{z\}[w]$ and that
$q_\nu(0)=0$ for every $\nu$.
Under these conditions Puiseux proved around 1850
that
...
employ its  structure as a $C^\infty$-manifold of dimension two.
...
The condition is intrinsic because it is a wellknown fact in the planar case 
that subarmonicity is unchanged under locally defined 
conformal mappings.
