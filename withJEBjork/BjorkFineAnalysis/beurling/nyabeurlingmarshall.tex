...
\noindent
We    shall expose results   by Beurling
which not only offer striking theorems whos proofs often are 
both  original and instructive. 
We have not 
ncluded material from his  deepest work  which  for example appear
in joint articles  with Lars Ahlfors, P. Deny  and Paul Malliavin. 
Two volumes devoted to
his collected work contains 
a foreword whivh gives a 
an  informative text  written by 
...
Many of his results also rely upon
solutions to variational probelms where
analytic function theory and Fourier analysis intervene in a fruitful  manner.
\medskip


\centerline{\emph{Beurling's students.}}
\medskip
...
thesis in 1950 at the age of 22 years. 
Among other ph.d. students from these yers one can mention
Sonja Lyttkens and Bo Kjellberg.
In addition to
students  at Uppsala university, Beurling  offered  gave
...
at Uppsala University. Carleman entered Uppsala in 1911 and Beurling in 1924.
During these  years the professors at Uppsala were
...
whose articles contain   Tauberian theorems with remaineder terms.
Preserved
...
basic results in functional analysis is assumed. 
Many  results in these notes are concerned with inequalities which
in the present author's opinion is  the core of mathematical analysis.
In order to  make these notes reasonably  self-contained we have included
...
pursue the details.
Let $u(z)$ be a continuous and subharmonic 
...
and that 
\[
u(e^{i\theta})\leq 0\quad\colon\quad 0<\theta<2\pi
\]
Assume in  addition   that there exists some $0<\alpha<1$ such that
\[
u(z)\leq |1-z|^{-\alpha}
\]
for every $z\in D$.
...
$D_\epsilon$, the principle of harmonic majorisation gives
$u\leq h$ in $D_\epsilon$.
So for  each $z\in D_\epsilon$ we get
\[
 u(z)\leq h(z)= \epsilon^{-\alpha}\cdot 
 \mathfrak{m}_{D_\epsilon}
(z_0;\mathcal C_\epsilon)\tag{i}
 \]
where
$ \mathfrak{m}_{D_\epsilon}
(z_0;\mathcal C_\epsilon)$ is the harmonic measure with respect to
$z_0$.
Next, one can easiuly check that for every
$r<1$ there exists a constant $C(r)$ such that if 
\[
|z_0|\leq e\,\&\, \epsilon\leq (1-r)/2\implies
\mathfrak{m}_{D_\epsilon}
...
\noindent
Let us announce a results which appears in Beurling's ph.d-thesis and
illustrates the flavout in the thesis.
...
\noindent
is constant as $z$ varies in $\Gamma$, where $|d\zeta|$ 
above denotes  the arc-length measure.
...
Inspired by this Beurling introduced a construction of 
conformal mappings which goes as follows:
Denote by $\mathcal C$ the family of
analytic functions $f$ in the open unit disc $D$ which
yield a conformal mapping onto some simply connected
domain where the derivative  $f'(0)$ is real and positive.
For each
...
{\bf A.1 Theorem.}
\emph{If  
$\log\,\frac{1}{\Phi(w)}$ is subharmonic, then 
$\mathcal C_\Phi$ contains a unique function
$f^*$.}
\medskip

\noindent
The proof occurs in § xx.
...
\noindent
{\bf{A.2. Invariant subspaces of $H^2(T)$.}}
...
\noindent
{\bf{A.3 Analytic extensions across a boundary.}}
Consider
the open square 
\[
...
\[
\int_{-\infty}^\infty\, \log^+\, \frac{1}{|g(x)|}\, dx<\infty
\]
...
It goes as follows: Consider an     $n$-tuple  of distinct points
$E=\{\alpha_1,\ldots,\alpha_n\}$ in the open unit disc $D$ and some $n$-tuple
$w_1,\ldots,w_n$ of complex numbers. Put
\[
...
For each finite
set $E$ as above  we set
\[
\lambda(E)= \max_{w}\, \rho(E;w(\cdot))
\]
with the maximum taken over all $n$-tuples $w$ in $D$.
In the article [1986]
Beurling considered a mini-max problem which 
gives  a new descripion of $\lambda(E)$.
Namely, given the   $n$-tuple $E=\{\alpha_\nu\}$ 
we set
\[
...
To prove Theorem A.4.2,   Beurling considered the following
Hilbert space problem:
First, consider  the Blaschke product
\[
B_E(z)= \prod_{\nu=1}^{\nu=n}
\,\frac{z-\alpha_\nu}{1-\bar \alpha_\nu\cdot z}\tag{**}
\]
and introduce the  discrete measure
\[
...
The results  above were established already in 1959, while the final part of the cited article
contains new matereial  which Beurling carried out in 1985,
...
\[ 
\lambda_nn\geq \delta\cdot n\tag{i}
\]
...
of exponential type, i.e. there are constants
$C$ and $A$ such that
...
the study of real-valued and integrable functions
$f(\theta)$ on the unit circle $T$
whose harmonic extensions $H_f$
to  the open unit disc $D$ have a finite Dirichlet integral.
The function $f\in L^1(T)$ 
...
\]
The sum in (*) is denoted by $\mathcal D(f)$
and  called the squared Dirichlet norm.
Given  some $f\in \mathcal D$ with constant term $a_0=0$
...
\]
With these notstions Beuling
studied the set of $\theta$ for which there exist the radial limit
\[
\lim_{r\to 1}\, f(r,\theta)\tag{**}
\]
A wellknown result due to Lebesgue asserts that
the radial limit exists  for almost every $\theta$. Moreover,
(0,1) is equal to $f(\theta)$ for every $\theta$ which is a Lebesgue point for the $L^1$-function $f$.
Following Beurling we shall exhibit a set of
$\theta$-values  where (0.1) exists
Namely, consider the partial derivatives of  with
respect to $r$ which becomes
\[
f'_r(r,\theta)=
\sum\uuu {n=1}^\infty\,n\
...
It is clear that if (3) is finite then  the radial limit in (**) exists.

\medskip

\noindent
Concering th limit funvtion $F^*$ we notice thst
It  takes value in $[0,+\infty]$ and as a limit of
a non-decreasing sequence of continuous functions it is lower
semi-continuous, i.e. the sets $\{F^*>a\}$ are open for every
$a\geq 0$.
It turns out that
$F^*\in L^1(T)$ 
and Beurling proved that much more can be said- More precisely one has
\medskip
...
D(F^*)\leq D(f)\tag{1.1}
\]
\emph{Moreover, when $\mathcal D(f)=1$ one has the inequality
below for every $\rho>0$}
\[ 
\text{Cap}(\{F^*>\rho\})\leq e^{\vvv \rho^2}\tag{1.2}
\]
\medskip


\noindent
{\bf{Remark.}} In (1.2) we have introduced the logarithmic capacity
of the open sets
$\{F^*>\rho\}$.
We refer to § xx below for some
properties about these capacities.
...
\centerline{\bf{1  On logarithmic potentials.}}
\medskip
...
{|z\vvv\zeta|}\cdot d\mu(\zeta)\tag{1.1}
\]
...
\tag{1.2}
\]
One says that $\mu$ has finite energy when (1.2) is finite.
To check when this holds
proceed as follows.
Put 
\[ 
h_n=\int\, \cos\,n\theta\cdot d\mu(\theta)\quad\&\quad
k_n=\int\, \sin\,n\theta\cdot d\mu(\theta)\tag{1.3}
\]
Next, with $r<1$ we notice that
\[
 \int \,U\uuu\mu(r,\phi)\cdot d\mu(\phi)
=\iint\, \log\, \frac{1}{|1\vvv re^{i(\phi\vvv\theta)}|}
d\mu(\phi)\cdot d\mu(\theta)\tag{1.4}
\]
...
h\uuu n\cos\,n\theta+
k\uuu n\sin\,n\theta)\tag{1.5}
\]
and conclude that
\[ 
J(\mu)=\lim_{r\to 1} \sum \frac{r^n}{n}\cdot (h^2\uuu n+k\uuu n^2)
\]
\medskip

\noindent
Hence $\mu$ has finite energy if and only if
\[
\sum \frac{1}{n}\cdot (h^2\uuu n+k\uuu n^2)<\infty\tag{1.6}
\]
\medskip
...
$\{a_n\}$ and $\{b_nn\}$ where $a_0=0$.
Assume in addition that
the $L^1$-function $f$ is lower semi-continuous. Then
\[
\int\, g\cdot d\mu=
\sum\, a\uuu n\cdot h\uuu n+b\uuu n\cdot k\uuu n\tag{1.7}
\]
and Cauchy\vvv Schwarz inequality gives:
\[
[\int\, g\cdot d\mu]^2\leq \mathcal D(g)\cdot J(\mu)\tag{1.8}
\]
...
\noindent
{\bf{1.1  Theorem.}}
\emph{For each  $\mu$ with finite energy and every
$g(\theta)\in\mathcal D$  which is lower semi-continuous
one has the 
inequality}

\[
\bigl[ \int\, g(\theta)\cdot d\mu(\theta)\, \bigr]^2\leq
\mathcal D(g)\cdot J(\mu)
\]
\bigskip


\noindent
{\bf{Remark.}} Above the lower semi-continuity on $g$ is imposed in order to
ensure that the usual Borel integral of $g$ with respect to
$\mu$ is defined.
...
\noindent
{\bf{2.1 Lemma.}} \emph{The function $F$ is subharmonic in $D$.}
\medskip
...
\noindent
{\bf{2.2  Dirichlet integrals.}}
Let $f\in \mathcal D$ with $a\uuu 0=0$ and
construct the Dirichlet integral of its harmonic extension:
\[
...
|G'(z)|^2\cdot dxdy
\]
Now the reader can verify that the double integral above is equal to $\mathcal D(f)$.
...
\bigl[\, r^2\cdot \bigl(\frac{\partial F}{\partial r}\bigr)^2+
\frac{1}{r^2}\cdot\bigl(\frac{\partial F}{\partial \theta}\bigr)^2\,\bigr]
\cdot r\cdot d\theta dr\tag{2.3.1}
\]
Show also that if $g$ is harmonic in $D$ then
\[
 \text{Dir}(g)= \frac{2}{\pi}\cdot 
\int\uuu 0^1\int\uuu 0^{2\pi}\, 
\bigl(\frac{\partial g}{\partial r}\bigr)^2
\cdot r\cdot d\theta dr\tag{2.3.2}
\]

\bigskip


\centerline{\emph{2.4  Proof of (1.1) in the Main Theorem}}
\bigskip

\noindent 
Apply (*) in 
Lemma 1.2, which after  
integration 
with respect
to $r$ gives
\[ 
...
\int\uuu 0^1\int\uuu 0^{2\pi}\, 
\bigl(\frac{\partial F}{\partial r}\bigr)^2\cdot r\cdot d\theta dr\tag{2.4.1}
\]
...
\] 
in the whole disc $D$.
Then (2.4.1) and the equality (2.3.2) applied to the harmonic function
$f$ give:
\[
\text{Dir}(F)\leq \text{Dir}(f)=\mathcal D(f)\tag{2}
\]
where the last equality used (*) in 2.2.
Next,  the harmonic extension of 
the boundary function $F^*(\theta)$ is  denoted by $H\uuu F$.
We have the equality
\[
\mathcal D(F^*)= \mathcal D(H\uuu F)\tag{3}
\]
Next, recall  that the Dirchlet integral is minimized when we take
a harmonic extension. This  entails that
\[
\text{Dir}(H_F)\leq
\text{Dir}(F)\tag{4}
\]
Hence (2\vvv 4) give the requested inequality (1.1) in the Main Theorem:
\[
\mathcal D(F^*)\leq \mathcal D(f)
\]
...
\centerline{\bf{2.5 Proof of (1.2) in the Main Theorem}}
...
\noindent
Let $\rho>0$ and let $\mu$ denote the
equilibrium 
distribution $\mu$ assigned to the
set $E=\{F^*>\rho\}$. Theorem 1.1.    gives
\[ 
\rho^2\leq \bigl[\int F^*\cdot d\mu\,\bigr]^2\leq \mathcal D(F^*)\cdot J(\mu)\tag{1}
\]
From § 2.4 we have  $\mathcal D(F^*)\leq \mathcal D(f)=1$ 
and hence 
\[
\rho^2\leq J(\mu)\tag{2}
\]
...
which proves (1.2) in the Main Theorem.
...
Moreover we assume that the area of $\Omega$ is equal
to $\pi$. Consider the harmonic measure
$\mathfrak{m}\uuu 0^\Omega(\ell)$, i.e.
the value at the origin of the harmonic function
...
$\phi(\alpha)=\ell$.
Recall that for an interval on the unit circle one has  the equality
\[
\text{Cap}(\alpha)= \sin\,\alpha/4\tag{3.2}
\]
At the same time
$\mathfrak{m}\uuu 0^D(\alpha)=\frac{\alpha}{2\pi}$ which entails that
\[
\mathfrak{m}\uuu 0^\Omega(\ell)=
\frac{2}{\pi}\,\arcsin\, \text{Cap}(\alpha)\tag{3.3}
\]


\noindent
To estimate  last term above we put
$u=\mathfrak{Re}\,\phi$.
...
\noindent
Let $E$ be a closed subset of the unit circle $T$. We 
...
\noindent
{\bf{Remark.}}
That  the condition (*) is crucial for the existence of smooth functions in
the disc algebra which are zero on $E$ can be seen via  
a uniqueness result due to Beurling which
goes as follows:
\medskip
...
$\alpha>0$
and is zero on $E$ must vanish identically.}
\medskip


\noindent
Proofs of the two theorems  above arppear  in § XXX.
Next,  consider a closed set
...
\noindent
Denote by $\mathcal O(\mathcal D)$ the class of analytic functions
...
Next we have
\[
\theta^s\cdot \int_\theta^1\, \rho(u)\cdot u^{-s-1}\, du=\frac{\theta^s}{s-1}\cdot
(\theta^{-s}-1)\tag{iv}
\]
Together (i-iv) give (*).
...
\noindent
Here we shall need
a closure theorem which we first establish  while the proof
...
K(\sigma+it)=0
\] 
As explained in §§ a  result due to  F. and R. Nevanlinna 
gives some constant 
...
\bigl(\int_{\gamma_2}\, \lambda^*(w)\, |dw|\bigr)^2\leq
...
\noindent
To prove
(2) we use a symmetrisation of $\lambda^*$.
Namely,
there
