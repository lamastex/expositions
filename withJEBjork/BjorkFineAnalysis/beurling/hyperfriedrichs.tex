...
doubly periodic continuously differentiable functions, and 
if 
$g\in C^k(\square)$
we set
\[ 
...
\] 
which in general can be strict.
...
the absolute value of (v) is majorized by
$C_Q\cdot ||f||_k\cdot ||g||_k$. It follows that
...
\noindent
{\bf{Sublemma.}} {\emph{ The operator  $B_k$ is self-adjoint}
\medskip
...
$g\in\mathcal H$.
From this the reader can check that (1) remains valid when
$g$ belongs to $\mathcal D(\mathcal T_k)$
which means that
\[
\langle Pf,g\rangle=
-\langle f,\mathcal T_kg\rangle+
\langle f,B_k(g)\rangle\quad\colon f\in C^\infty(\square)\tag{2}
\]
...
we can
reverse their positions in (1) which gives
\[
\langle Pg,f\rangle=
-\langle g,Pf\rangle+
\langle g,B_k(f)\rangle\tag{3}
\]
Since $a$ and $b$ are real-valued it is clear that
\[
\langle Pg,f\rangle=-\langle f,Pg\rangle\tag{4}
\]
It follows that
\[
\langle f,B_k(g)=\langle g,B_k(f)\quad\colon f,g\in C^\infty(\square)\tag{5}
\]
...
\noindent
Next follows a crucial step towards the proof of Proposition A.1.
\bigskip

\noindent
{\bf{Sulemma 2.}} \emph{One has the 
equality 
$\mathcal D(T_k^*)=
\mathcal D_k(P)$. Moreover one has the equality of operators}
\[
T_k^*=-\mathcal T_k(g)+B_k\tag{*}
\] 
...
\[
|\langle Pf,g\rangle|\leq C\cdot ||f||\quad\colon f\in C^\infty(\square)\tag{1}
\]
Since $B_k$ is a bounded operator, the equation (3) from the proof of
Sublemma 1 
gives the inclusion
\[
...
(3) for the proof of the Sublemma  entails that
...
which proves Sublemma
1 
and at the same time the requested operator equation
\[
T_k^*=-\mathcal T_k(g)+B_k
\] 
and the proof of Sublemma 2 is finished.
\bigskip
...
\]
Then (*) in Sublemma 2 implies that (i) is zero and hence we have
proved 
\[
...
\[
||T_k(f)-\lambda\cdot f||\geq
\frac{\lambda}{2}\cdot ||f||\quad\colon f\in\mathcal D(T_k)
\]
\medskip
...
\]
Since $g\in\mathcal H$ was arbitrary theorem  Theorem A follows.

\bigskip

\noindent{\bf{3.1 Density of the range.}}
...
\[ 
\tau=\min_f\,||B(f)||\quad\colon\quad
M=\frac{1}{2}\cdot \max_{(x,s )\in\square}\,|\partial_x(a)(x,s)|
\]
With these two positive constants we prove:
\bigskip

\noindent{\bf{3.2 Proposition}}
\emph{For each $\lambda>M+\tau$ it follows that
$T_k^*(f)-\lambda\cdot E$ is injective.}
\medskip


\noindent
\emph{Proof.} Let $\lambda>M+\tau$ and suppose that
\[ 
...
\]
From (i) and Proposition ¤ XX we have
\[
...
\[
(\lambda-M)\cdot \int_0^\pi V(s)\, ds=\int_{-\pi}^\pi \Phi(s)\,ds \leq ||B(f)||\tag{vii}
\]
Finally, by the normalisation (i) we have
$\int_0i^\pi V(s)\, ds=1$ so (vii) entails that we mst have
\[
\lambda\leq M+||B(f)||\tag{viii}
\]
