...
\noindent
{\bf{Introduction.}}
The main result is Theorem xx below which was proved  in
[Beurling]. First 
we
insert comments from Beurling's ariticle
about 
the  significance of Theorem XX below.
...
de la mŽcanique quantique} published by
Schršdinger in 1932
raised a new and unorthodox question concerning Brownian motions
...
\]
where $u_0$ is a solution to (*) above
defined for $t>0$
...
Namely, let
$f\uuu 0,f\uuu 1$ be a pair of  non-negative functions
such that
\[
\int\uuu\Omega\, f\uuu 0\cdot dx=
\int\uuu\Omega\, f\uuu 1\cdot dx
\]
Then one asks if there
exists a unique 
pair $(u_1,u_2)$ as above such that
the product function $w$ satisfies
\[ 
...
An account about (eventual) mathematical solutions to
Schršdinger equations was presented
by I.N. Bernstein in a plenary lecture
at  the IMU\vvv congress at ZŸrich 1932.
A first example appears
on the product
...
\[
g(t)= \frac{1}{\sqrt{2\pi}}\cdot e^{\vvv t^2/2}
\]
...
Denote by
$\mathcal S^*$ the class 
of  non\vvv negative product measures
$\gamma\uuu 1\times\gamma\uuu 2 $ on ${\bf{R}}^2$
for which
\[
\iint g(x\uuu 1\vvv x\uuu 2)\cdot d\gamma\uuu 1(x\uuu 1)
\cdot d\gamma\uuu 2(x\uuu 2)=1\tag{*}
\]
...
a unique $\gamma\uuu 1\times\gamma\uuu 2$ in $\mathcal S^*$
such that} 
\[
...
\centerline{\bf\large{0. An automorphism on product measures}}
...
$\prod\,\Omega\uuu\nu$ and   its Boolean $\sigma$\vvv algebra 
is generated by
$\{\mathcal  B\uuu \nu\}$.
\medskip
...
and
refer to $\gamma^*$ as the product measure. It is
uniquely determined  because the Boolean $\sigma$-algebra  $\mathcal B$ 
on $Y$ by definition is generated
by
...
The condition that $\mu$ is a product measure is that there exist
...
\noindent
{\bf{0.2 The operator $T\uuu k$.}}
With a fixed $1\leq\nu\leq n$ we consider 
$\mathcal B\uuu\nu$\vvv measurable functions $g_\nu$.
Every such $g_\nu$
yields
 the function on $Y$
defined by
\[
g_\nu^*(x\uuu 1,\ldots,x\uuu n)=g_\nu(x\uuu n)
\]
...
both are bounded functions and let
$\mu$ be a non\vvv negative
product measure on $Y$ such that
\[ 
\int\uuu Y\, k\cdot d\mu=1\tag{i}
\]

\medskip

\noindent
For each $\nu$ we find a unqiue non-negative masrue on $X_\nu$
denoted by $(k\mu)_\nu$ where
\[
\int_{X_\nu}\, g_\nu\cdot (k\mu)\uuu\nu=\int\uuu Y\, g_\nu^*\cdot k\cdot d\mu\tag{ii}
\]
hold for all bounded measure functions on $X_\nu$.
Now we construct
the product measure
\[ 
...
satisfying (i)  and  let $\mathcal S^*\uuu 1$ be the family
of
...
\noindent
{\bf{Main Theorem.}}
\emph{$T_k$ yields a homeomorphism between $S^*\uuu k$ and $S^*\uuu 1$}.
...
Recall from XX that the space of measures on $Y$
...
\noindent
{\bf{Exercise.}}
Verify that $T_k$ is a continuous and injective map from
$\mathcal S^*\uuu k$ to $\mathcal S^*\uuu 1$.
So the main theorem anounts to prove that $T_k$ is surjective which
will
be achieved in ¤ 0.4 below.
\bigskip


\centerline {\bf{0.4 A variational problem.}}
\medskip

\noindent
The proof  that $T_k$ is surjective
relies upon a variational
problem.
Before it is presented we insert a preliminry result which plays an essential role 
later on.
...
\noindent
{\bf{0.6 The linear space $\mathcal A$}}.
It denotes 
the class 
of functions on $Y$
of the form
...
$a\in \mathcal A$  we put
\[ 
W(a,\gamma)=\int\uuu Y\, (e^ak\vvv a)\cdot d\gamma \tag{0.6.1}
\]
...
\[
W\uuu *(\gamma)=
 \min\uuu{a\in\mathcal A}\,W(a,\gamma)\tag{0.6.2}
\]
...
\]
It follows that 
$W(a,\gamma)\geq 1+\log k\uuu *$
where $k\uuu *$ is the minium of the positive $k$\vvv function and hence that
\[
W_*(\gamma)\geq 1+\log k\uuu *\tag{*}
\]
\medskip

\noindent
The requested surjectivity of $T_k$
follows from the following:
\medskip

\noindent
{\bf{0.7 Proposition.}} \emph{Let $\{a\uuu \nu\}$ be a sequence in
$\mathcal A$ such that}
\[
\lim W(\gamma,a\uuu\nu)= W\uuu*(\gamma)
\]
...
$W(a,\gamma)\leq m\uuu *(\gamma)+\epsilon$. Then }
...
$W(a\vvv s,\gamma)\geq W(a,\gamma)\vvv \epsilon$. This entails that
...
\noindent
Hence Lemma 0.8  follows with  $s=1$.
...
\centerline {\emph{Proof of Proposition 0.7}}
\bigskip


\noindent
Let $0<\epsilon<1$ and consider a pair $a,b$ in $\mathcal A$ such that
$W(a,\gamma)$ and $W(b,\gamma )$ both are $\leq W\uuu *(\gamma)+ \epsilon$.
The inclusion $\frac{1}{2}(a+b)\in\mathcal A$ gives
\[
2\cdot W(\frac{1}{2}(a+b),\gamma)\geq 2\cdot m\uuu *(\gamma)
\geq W(a,\gamma)+W(b,\gamma)\vvv 2\epsilon\tag{i}
\]
Notice that
\[
W(a,\gamma)+W(b,\gamma)\vvv
...
\[
\int_ Y\, (e^{a/2}\vvv e^{b/2})^2\cdot k\cdot d\gamma \leq 2\epsilon\tag{iv}
\]
...
2\epsilon\cdot \int\uuu Y\, (e^{a/2}+e^{b/2})\cdot k\cdot d\gamma\tag{v}
\]
Lemma 0.8
implies
that the last factor is bounded by
a fixed constant and hence there exists a constant $C$ such that
\[
...
we get
\[
||e^a\cdot \gamma-e^b\cdot \gamma||=
\int\uuu Y\, |e^a\vvv e^b|\cdot d\gamma
\leq C\cdot \sqrt{\epsilon}\tag{vii}
\] 
Since this hold for every $\epsilon$ it follows that when
$\{a_\nu\}$ is a sequence such that
\[ 
\lim\, W(a_\nu,\gamma)= W_*(\gamma)
\]
then
$\{e^a\cdot\gamma\}$
is a Cauchy sequence and therefore
converges to a limit
measure $\mu$
where

\[
\lim_{\nu\to\infty}\, ||e^{a\uuu\nu}\cdot\gamma\vvv \mu||\to 0\tag{viii}
\]
\newpage



\centerline{\emph{0.9 The equality $T(\mu)=\gamma$}}.
\medskip


\noindent
Consider some $\rho\in \mathcal A$ whose maximum norm
$|\rho|_Y\leq 1$
which enable us to write
\[
e^{-\rho}=1-\rho+\rho\uuu 1\quad\colon\, 0\leq \rho\uuu 1\leq \rho^2\tag{1}
\]
...
Hence we have
\[
\int_Y\, \rho(e^{ka_\nu}-1)\cdot d\gamma\leq \epsilon_\nu+
\int_Y\, e^{k a_\nu}\rho_1\cdot d\gamma\tag{3}
\]
Next, we have a fixed constant $C$ such that
\[
\int_Y\, e^{k a_\nu}\cdot d\gamma\leq C
\]
So (3) entails that
\[
\int\uuu Y\,(e^{ka_\nu}-1)\cdot \rho]\cdot d\gamma\leq
\epsilon\uuu\nu +C\cdot ||\rho||_ Y^2\tag{4}
\]
The same inequality  with $\rho$ replaced by $\vvv \rho$
which entails that
\[
\bigl|\int\uuu Y\,(ke^{a\uuu\nu}\vvv 1)\cdot \rho\cdot d\gamma\bigr|
\leq\epsilon\uuu\nu+C\cdot ||\rho||\uuu Y^2
\]
At this stage we apply Lemma 0.5 to the measure
...
(\epsilon_\nu+C\epsilon_\nu)= n(1+C)\cdot \sqrt{\epsilon\uuu\nu}
\]
Passing to the limit we get the requested equality in ¤ 0.9 and
Proposition 0.7 is proved.
