...
\centerline{\bf\large  A system of infinite linear equations.}
...
the system
\[ 
\sum_{q\neq p}\,\frac{x_q}{p-q}=0\tag{*}
\] 
where (*) hold for all positive integers $p$ and
$\{x\uuu q\}$ is a sequence of real numbers for which the series
\[ 
\sum\uuu{q=1}^\infty\,\frac{x\uuu q}{q}=1\tag{**}
\] 
under the sole assumption that the series in the left hand side is convergent.
Already the
fact that (*) has a non-trivial solution is
far from evident.
Before the study of (*)  in Section 1 we
discuss a  general situation  which was described
by Torsten Carleman in his major lecture at
the international congress in ZŸrich 1932.
...
\]
Following Carleman the generic case occurs when
...
\noindent
The proof of  uniquenss relies upon Jensen's formula and the existence upon 
the solution to  a  Wiener-Hopf
equation.
...
$\mathfrak{Re}(z)\leq 0$ for some constant $C_*$.
\[
\lim_{x\to -\infty}\,h(x)=0\quad\&\quad
|h(x+iy)|\leq C_*\quad\colon\,x\leq 0\tag{i}
\]
Moreover, in the right half-plane there exists  a constant $C^*$ such that
\[
|h(x+iy)|\leq C^*\cdot \frac{|x|}{1+|y|}\quad\colon\, \quad |x-q|\geq \frac{1}{2}\,\,
\text{hold for all positive integers}\tag{ii}
\]

\noindent
To $h(z)$ we get the entire function $H(z)$ as in ¤ 1
and  (i-ii) above give the two
the estimates below in the right half-plane:
\[ 
|H(x+iy)|\leq Ce^{\pi|y|}\,\,\colon\, x\leq 0\quad\&\,\quad
...
for the entire function $H'(z)$ and an  application
of
the  PhragmŽn-Lindelšf theorem 
gives:
\medskip
...
\[
\lim_{r\to\infty}\,\max_{0\leq \phi\leq 2\pi}\,
e^{-\pi r\cdot |\text{sin}\,\phi|}\cdot |H'(re^{i\theta}\,|=0
\]
\medskip

\noindent
Now we are prepared to prove the uniqueness part in Theorem 1.1.
...
(*), the second equation  in Proposition 1.2 gives
\[ 
...
e^{-i\theta z}\cdot \theta\cdot \phi(e^{i\theta})\,d\theta\tag{2}
\]
\medskip
...
\int_{\Gamma_\epsilon}\,\frac{\phi(\zeta)\,d\zeta}{\zeta^{n+1}}\tag{*}
\]
\medskip
...
\[
-\frac{1}{2\pi i}\int_{\gamma_\epsilon}\,
\frac{e^{-z\cdot\text{Log}\zeta}\cdot\phi(\zeta)}{\zeta}\,d\zeta
 \tag{ii}
\]
...
\noindent 
{\bf 2.3 Conclusion.}
If $n$ is a positive integer in Proposition 2.2 we have:
\[ 
F'(n)=
...
$|z|>1$. We are going to  show that
this is true for a specific $\phi$-function 
satisfying the growth condition (1) above and in addition
the series
\[ 
\sum_{n=1}^\infty\, (-1)^n\cdot \frac{c_ n}{n}
\]
converges.
\bigskip
...
From (i) in Proposition  1.1 and Proposition 2.2 we get

\[ 
H(p)=(-1)^p\cdot x_ p=c_p=F(p)
\]
...
\]
for all $q\geq 1$.
By (ii) in Proposition 1.2 this means precisely that
...
\[
\log |\psi(e^{i\theta})|=- \mathfrak{Re}\, g(e^{i\theta})=
\vvv u(\theta)
\]
Since $2u(\theta)= \vvv \log\,|\theta|$
it follows that
\[
\log\,|\theta|+\log |\phi_ *(e^{i\theta})|=\log |\psi(e^{i\theta})|
\]
...
\[
|\theta|\cdot \phi_ *(e^{i\theta})= \psi(e^{i\theta})\tag{*}
\]
...
\[ \phi_1(z)=
\frac{1}{\sqrt{1-z^2}}\quad\text{and}\quad \, \psi_1(z)=
\frac{1}{\sqrt{1-z^{-2}}}
\]
\medskip
...
\[
\rho(\theta)=\frac{\phi_1(e^{i\theta})}
{\psi_1(e^{i\theta})}
\] 
when $- \pi<\theta <\pi$ and $\theta\neq 0$.
...
it is defined by
\[ 
\phi(z)=\frac{z}{\sqrt{1- z^2}}\cdot \phi_ *(z)
\]
From (*) above and the construction of $\rho$
it follows that
\[ 
\theta\cdot \phi(e^{i\theta})=
\frac{\pi}{2}\cdot  \psi_1(e^{i\theta})\cdot\psi(e^{i\theta})
\]
...
\frac{e^{i\theta}+z}{e^{i\theta}-z}\cdot k(e^{i\theta})\, d\theta\tag{vi}
\]
yields a 
...
{\bf{Lemma.}} The series
\[
\sum_{p=1}^\infty\, (-1)^n\cdot\frac{c_ n}{n}
\]
is convergent.
\medskip
...
\[
\phi(z)=\frac{A(1)}{1\vvv z}+\frac{A(z)-A(1)}{1- z}
\]
