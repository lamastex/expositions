...
due to Carleman, Lindelšf and MŸntz.
...
Lindelšf proved that  bounds exist
for many different  norms on
the given function $f$ 
in the
article 
\emph{Sur un principe gŽneral de l'analyse et ses applications
ˆ la theorie de la reprŽsentation conforme}
from 1915.
Let us  announce  two results from [Lindelšf]
of this nature.
...
\]
With these notations Lindelšf used the following lemma whose proof
is  left as
an exercise:
\medskip
...
established later by Lindelšf and  De VallŽ-Poussin.
We shall not pursue this any further.
The reader can
consult
articles by  De VallŽ-Poussin which contain
...
Lemma 6.6 gives a sequence $\{Q_n\}$ which approximates
...
\centerline{\bf D. Theorem of MŸntz}
\bigskip
...
is due to MŸntz. See his article
...
\[
\int\uuu 0^1\, e^{+\vvv im\uuu n x}\cdot f(x)\cdot dx=0\tag{**}
\]
...
\]
We leave it to the reader to verify that it suffices to
prove this version of  Theorem D.3.
With  $f$ as above we set
\[
\phi(z)= \int\uuu{\vvv \pi}^\pi\, e^{izt}\cdot f(t)\cdot dt\tag{ii}
\]
We see that the entire function $\phi$ is even and (i) means that
$\phi(0)=1$.
Moreover, $\phi$  is an entire function of exponential type
and Cauchy\vvv Schwarz inequality gives
\[
|\phi(x+iy)|\leq ||f||\uuu 2\cdot e^{\pi|y|}/tag{iii}
\]
for all $z=x+iy$.
...
\noindent
Next, since 
the zeros of $\phi$ contains the even sequence $\{+\vvv m\uuu\nu\}$
...
a non\vvv zero function  $\phi$ and hence also $f\neq 0$ cannot
exist.
