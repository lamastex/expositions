...
Carelman's result in
Theorem A.1 was inspired 
by earlier constructions due to Lindelöf.
...
\noindent
Thus, we have taken a two-fold logarithm which  means 
that $\omega(\phi)$ is considerably smaller  compared
to the $\rho$-function.
\medskip
...
Put
\[ v(r)=\text{Log}\, M(r)\quad\colon\quad
U(z)=\text{Log}\,|f(z)|\tag{i}
\]
Given $r$ we consider the domain
\[
...
identically zero.
Via a conformal map from the half-disc $D_+$ above 
...
A complete answer to this uniqueness problem
was proved by Carleman in [Car].
First we exhibit a class of functions
whose asymptotic series vanish identically.
Namely. consider a sequence of real positive numbers
$A_1,A_2,\ldots$.
To each $n\geq 1$ we put
\[ 
I_n=\text{exp}\bigl(\, \frac{1}{\pi}\int_1^\infty\, \text{Log}\,
\bigr[\,\sum_{\nu=1}^{\nu=n}\,
\frac{r^{2\nu}}{A_\nu2}\,\bigr]\, \cdot dr\,\bigr)\tag{**}
\]
\medskip
...
$\{A_n\}$ such that the associated sequence
...
\noindent
{\bf B.5. A converse result.}
In [loc.cit ] appears also  proof of  the converse to the
result above. 
\medskip
...
satisfies (***) in Theorem 4. Then 
$\{ A_n\}\in\mathfrak{B}$.}
\bigskip

\noindent
The proof of the two results above rely upon
a varational
problem which
is presented below 
while the
...
are the zeros in the exterior disc. Thus
\[ 
\Omega(z)=z^{-n}\cdot \frac{(-1)^n}{a_0^2}\cdot
p_n(z)\cdot\prod\,
(z-\frac{1}{\rho_\nu})\quad\colon\,
...
\noindent
{\bf C.1 Theorem.}\emph{
The variational problem has a unique solution where
the minimum $I_*$ is achieved by the function}
\[ 
f_*(x)=(1-z)^n\cdot \frac{1}{\prod\, 1-\rho_\nu\cdot z)}
\]
Moreover, 
\[ 
I_*=
...
\[ 
I(\phi)=\frac{1}{2\pi}\cdot\int_0^{2\pi}\,
\Omega(e^{i\theta})\cdot |\phi(e^{i\theta})|^2\cdot d\theta\quad\colon
\phi(0)=1\tag{1}
\]
With $f_*$ as in the theorem we get the $\phi$-function
...
\noindent
Now $f_*$ is a unique minimizing function if we have proved the strict 
inequality
\[
I(\phi_*+h)<I(\phi_*)\tag{3}
\] 
for every analytic function $h(z)$ in $D$ with $h(0)=0$.
To show this we 
...
\Omega(z)\cdot |\phi(z)+h(z)|^2\cdot \frac{dz}{z}=
\]
\[
I(\phi_*+I(h)+
\frac{1}{2\pi i}
...
\noindent
Since $\Omega=\bar\Omega$ holds on $|z|=1$ where we also have
$\bar z= z^{-1}$, 
it follows from (*) that
$|z|=1$ entails
\[
\Omega(z)=z^n\cdot \frac{(-1)^n}{a_0^2}\cdot
\prod\,(\frac{1}{z}-\frac{1}{\rho_\nu})\cdot
\prod\,(\frac{1}{z}-\rho_\nu)=
\]
\[
\frac{(-1)^n}{a_0^2}\cdot\prod\,(1-z\rho_\nu)\cdot
\prod\,(\frac{\rho_\nu}{z}-1)\cdot \frac{1}{\rho_1\cdots \rho_n}
\tag{ii}
\] 



At the same time (2) gives
\[
\bar\phi_*(z)=
 \bigl(\prod\, (1-\frac{\rho_\nu}{z})\bigr)^{-1} \tag{ii}
\]
Hence
\[
\Omega(z)\cdot \bar\phi_*(z)=\frac{1}{\rho_1\cdot \rho_n\cdot a_0^2}\cdot 
\prod\,(1-\rho_\nu\cdot z)
\]
Since $h(0)=0$ it follows that
\[
\int_{|z|=1}\,
\Omega(z)\cdot \bar\phi_*(z)\cdot h(z)\cdot \frac{dz}{z}=0
\]
In the same way we get
\[
\int_{|z|=1}\,
\Omega(z)\cdot \phi_*(z)\cdot \bar h(z)\cdot \frac{dz}{z}=0
\]
Hence the last integral in (4) is zero
which shows that
\[
I(\phi_*+h)=
I(\phi_*+I(h)
\]
and since $I(h)>0$ we get the strict inequality in (3).
